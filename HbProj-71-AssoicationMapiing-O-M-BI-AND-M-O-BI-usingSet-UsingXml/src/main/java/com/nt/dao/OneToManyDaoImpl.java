	...
	public void saveDataChildToParent() {
		...
		try {
			if (!ses.getTransaction().isActive())
				tx = ses.beginTransaction();
			//set parent object
			UserInfo info = new UserInfo("finch", "saudi");
			//set child  object data
			PhoneNumber phone1 = new PhoneNumber(5555555l, "prepaierd", "VI");
			PhoneNumber phone2 = new PhoneNumber(4444444l, "postpaid", "jio");
			phone1.setUser(info);
			phone2.setUser(info);
			Set<PhoneNumber> phoneSet = Set.of(phone1, phone2);
			info.setPhoneDetails(phoneSet);
			//save data
			ses.save(phone1);ses.save(phone2);
			flag = true;
